{
  config,
  pkgs,
  ...
}:
{
  imports = [
    ./hyprwm.nix
    ./nvim.nix
    ./zsh.nix
    ./firefox.nix
  ];
  home.username = "luke";
  home.homeDirectory = "/home/luke";
  home.packages = with pkgs; [
    unzip
    hyprlauncher
    nixfmt-rfc-style
    qq
    (wechat.overrideAttrs {
      src = fetchurl {
        url = "https://dldir1v6.qq.com/weixin/Universal/Linux/WeChatLinux_x86_64.AppImage";
        hash = "sha256-+r5Ebu40GVGG2m2lmCFQ/JkiDsN/u7XEtnLrB98602w=";
      };
    })
  ];
  nixpkgs.config = import ./nixpkgs.nix;
  xdg = {
    enable = true;
    configFile = {
      "nixpkgs/config.nix".source = config.lib.file.mkOutOfStoreSymlink ./nixpkgs.nix;
    };
    dataFile = {
      "fcitx5/rime/default.custom.yaml".source =
        config.lib.file.mkOutOfStoreSymlink ./src/rime/default.custom.yaml;
      "fcitx5/rime/flypy.custom.yaml".source =
        config.lib.file.mkOutOfStoreSymlink ./src/rime/flypy.custom.yaml;
      "fcitx5/rime/flypy.custom.dict.yaml".source =
        config.lib.file.mkOutOfStoreSymlink ./src/rime/flypy.custom.dict.yaml;
    };
  };
  wayland = {
    systemd = {
      target = "hyprland-session.target";
    };
  };
  programs = {
    yazi = {
      enable = true;
      enableZshIntegration = true;
      shellWrapperName = "yz";
      plugins = with pkgs.yaziPlugins; {
        git = git;
        sudo = sudo;
        diff = diff;
        mount = mount;
        chmod = chmod;
        starship = starship;
        vcs-files = vcs-files;
        bookmarks = bookmarks;
      };
      settings = {
        mgr = {
          show_hidden = true;
        };
      };
    };
    rofi = {
      enable = true;
    };
    ripgrep = {
      enable = true;
    };
    git = {
      enable = true;
    };
    kitty = {
      enable = true;
      font.name = "Maple Mono Normal NL NF CN";
      themeFile = "OneDark-Pro";
      settings = {
        cursor_trail = 1;
        background_opacity = 0.7;
      };
    };
  };
  i18n = {
    inputMethod = {
      enable = true;
      type = "fcitx5";
      fcitx5 = {
        waylandFrontend = true;
        addons = with pkgs; [
          fcitx5-material-color
          (fcitx5-rime.override {
            rimeDataPkgs = [
              (pkgs.callPackage ./rime-flypy.nix { })
            ];
          })
        ];
        settings = {
          inputMethod = {
            GroupOrder."0" = "Default";
            "Groups/0" = {
              Name = "Default";
              "Default Layout" = "us";
              DefaultIM = "rime";
            };
            "Groups/0/Items/0".Name = "keyboard-us";
            "Groups/0/Items/1".Name = "rime";
          };
          addons = {
            classicui = {
              globalSection = {
                Theme = "Material-Color-sakuraPink";
              };
            };
          };
        };
      };
    };
  };
  home.stateVersion = "25.11";
}
